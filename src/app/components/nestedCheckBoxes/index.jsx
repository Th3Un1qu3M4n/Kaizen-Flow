...
function traverseAndToggleNode(data, value, key) {
	let toggled = false;
...
export function CheckItem({ node, setData }) {
	...
	return (
		<div key={"wrapper_" + node.value} style={{ width: "100%" }}>
			...
			>
				<label
					style={{
						color: node.isDisabled ? "gray" : "white",
						display: "flex",
						flexDirection: "row",
          ...
          /> */}
				...
				<div>
					<div>
						{(node.isHeading || node.isSubHeading) && (
							<>
								<input
									type="text"
									value={newNodeLabel}
									...
									style={{
										...
										padding: "5px",
										borderRadius: "5px",
										fontSize: node.isHeading ? "15px" : "10px",
									}}
								/>
								<button
									style={{
										border: "1px solid #7A00F3",
										backgroundColor: node.isHeading ? "#7A00F3" : "transparent",
										fontSize: "12px",
										fontWeight: "700",
										color: "#FFFFFF",
										padding: "8px",
									...
									onClick={addNewNode}
								>
									{node.value != "BigGoal" ? "Subtask +" : "Task +"}
								</button>
							</>
						)}
						{node.value != "BigGoal" && (
...
export function NestedChecklist({ data, setData }) {
	return (
		<>
			{data.map((node) => (
